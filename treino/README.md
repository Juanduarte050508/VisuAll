...
| Gravei errado | apague o arquivo em `data\raw_videos\<LETRA>\` (o nome tem data e hora) |
...
que já existe na pasta `data\`, inclusive o que você gravou semanas atrás.
...
**Não apague a pasta `data\`.** É o seu acervo. Sem ela, não dá pra
retreinar.
...
```
data\raw_images\<LETRA>\        fotos das letras paradas
data\raw_videos\<LETRA>\        vídeos das letras com movimento
data\raw_body_videos\<GESTO>\   vídeos dos gestos corporais
data\raw_negativos\NADA\        vídeos de "não é sinal nenhum"

mobile\app\src\main\assets\     os modelos que o app usa
treino\modelos_anteriores\      backups automáticos (o RestaurarModelo usa)
```

Detalhes técnicos (formatos, contrato com o app, como funciona por dentro)
estão no **`TREINO.md`**, na pasta de cima.
