...
subproduto do treino e usados pelo backend de PC (`linear/` e `modular/`).
Eles ficam versionados aqui quando fazem parte do funcionamento do desktop.
Backups, modelos antigos e datasets continuam fora do Git.

O app Android nao le estes `.pkl`; ele usa os `.onnx`/`.tflite` em
`mobile/app/src/main/assets/`. Se voce retreinar o desktop, confira se a
mudanca nos `.pkl` deve acompanhar a branch.
...
Para treinar ou retreinar, veja [`../treino/README.md`](../treino/README.md).
...
[`../treino/exportar_onnx.py`](../treino/exportar_onnx.py) (letras) e
[`../treino/treinar_corpo.py`](../treino/treinar_corpo.py) (gestos). A
