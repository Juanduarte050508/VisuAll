...
Salva exatamente onde o pipeline atual deste repo le:
  ...
  - letra DINAMICA (H J K X Z) -> computer/data/raw_videos/<LETRA>/AAAAMMDD_HHMMSS.mp4
    (lido por linear/backend/data_extraction/extract_from_videos.py)

  - letra ESTATICA (A B C ...) -> computer/data/raw_images/<LETRA>/AAAAMMDD_HHMMSS_NN.jpg
  ...
  - NADA (nao e sinal nenhum)  -> computer/data/raw_negativos/NADA/AAAAMMDD_HHMMSS.mp4
    (lido por computer/treino/extrair_negativos.py; sao os exemplos de "isto NAO e
    letra" que os modelos individuais precisam pra parar de reconhecer letra
    onde nao tem)

Nao depende de nada alem do que ja esta em computer/requirements.txt (opencv-python).
...
# As mesmas classes que estao dentro de computer/models/static_classes.pkl e
# computer/models/dynamic_classes.pkl -- gravar um rotulo fora dessas listas gera
...
_LABELS_CORPO = (REPO / "mobile" / "app" / "src" / "main" / "assets"
...
def main():
    ...
    print("\nPronto. Agora rode computer\\treino\\Treinar.bat pra gerar os modelos.")
    return 0
