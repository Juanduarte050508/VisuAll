...
Destino (o mesmo que computer/treino/gravar.py usa):
  letras dinamicas H J K X Z -> computer/data/raw_videos/<LETRA>/*.mp4
  letras estaticas A B C ... -> computer/data/raw_images/<LETRA>/*.jpg
...
Uso:
  ...
  python treino/importar.py "D:/DCIM/VisuAll"

  # 2) celular Android conectado por cabo (precisa do adb no PATH)
  python treino/importar.py --adb
...
def main():
    ...
    if relatorio["ignorados"]:
        ...
        for linha in relatorio["ignorados"]:
            print("  " + linha)
    print("\nDestino: %s" % DATA)
    print("Proximo passo: computer\\treino\\Treinar.bat")
    return 0
