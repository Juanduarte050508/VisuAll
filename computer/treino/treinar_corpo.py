...
Entrada:  computer/data/raw_body_videos/<GESTO>/*.mp4   (gravados no modo "corpo")
...
ASSETS = REPO / "mobile" / "app" / "src" / "main" / "assets" / "gestos" / "geral"
...
def normaliza_corpo(frame):
    ...
    # computer/tests/fixtures/landmark_contract.json dependendo da maquina.
...
def main():
    ...
    if faltando and not args.do_zero:
        ...
        if professor is None:
            ...
            print("  " + "!" * 58)
            print("  O arquivo %s" % (ASSETS / "model.tflite").relative_to(REPO))
    ...
    print("  gerado: %s (%.0f KB)"
          % (destino.relative_to(REPO), destino.stat().st_size / 1024))
    print("  gerado: %s -> %s" % (labels_path.relative_to(REPO), " ".join(classes)))
