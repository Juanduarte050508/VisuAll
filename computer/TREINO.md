...
Este arquivo fica em `computer/`; os comandos Python abaixo assumem que voce
esta nessa pasta.
...
O APK sai em `app\build\outputs\apk\debug\app-debug.apk`. Instale e pronto
...
**Ao commitar:** os `.onnx`/`.tflite` dos assets do app são fonte do aplicativo;
o APK/AAB gerado fica ignorado pelo `.gitignore` porque é pesado e recriável.
